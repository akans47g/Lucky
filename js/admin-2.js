...
window.executeApprove = async () => {
  ...
  try {
    ...
    /* Try by UID first */
    if(uid && uid !== 'undefined' && uid.length > 5) {
      const snap = await getDoc(doc(db,'users',uid));
      if(snap.exists()) {
        userRef  = doc(db,'users',uid);
        curCoins = snap.data().coins || 0;
      }
    }

    /* Fallback: search by email in users collection */
    if(!userRef) {
      const email = document.getElementById('ap-user').textContent;
      if(email && email !== '—') {
        const q    = query(collection(db,'users'), where('email','==',email), limit(1));
        const snap = await getDocs(q);
        if(!snap.empty) {
          userRef  = snap.docs[0].ref;
          curCoins = snap.docs[0].data().coins || 0;
        }
      }
    }
    ...
    /* ── Step 2: Update coins ── */
    if(userRef) {
      /* User doc milaa — coins add karo */
      await updateDoc(userRef, {
        coins:     curCoins + coins,
        updatedAt: serverTimestamp()
      });
    } else {
      /* User doc nahi mila — UID se naya doc banao */
      if(uid && uid !== 'undefined' && uid.length > 5) {
        const email = document.getElementById('ap-user').textContent;
        ...
        /* UID bhi nahi — request approve karo par coins note mein likho */
        toast('⚠️ User document nahi mila — coins manually add karo Users tab se', 'w');
      }
    }
    ...
    toast(`✅ ₹${amount} approved! ${coins} coins add ho gaye.`, 's');
    loadRequests();
    loadDashboard();
    loadUsers();
